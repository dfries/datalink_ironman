...
main( int argc, char **argv )
{
	...
	if ( cur == 0 ) {
	    fprintf( stderr, "Could not allocate packet memory.\n" );
	    exit( -1 );
	    }
	...
	if ( cur == 0 ) {
	    fprintf( stderr, "Could not allocate packet data memory.\n" );
	    exit( -1 );
	    }
	...
	DefaultRootWindow( display ), visualInfo->visual,
	AllocNone );
	...
	CWBorderPixel | CWColormap | CWOverrideRedirect, &attribs );
