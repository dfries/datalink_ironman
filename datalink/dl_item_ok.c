...
ItemPtr ip;
{
	...
	case DL_TIME_TYPE:
		...
		if (ip->data.time.tz_num < 1 || ip->data.time.tz_num > wi->max_tz) {
		...
		if (ip->data.time.hours > 23) {
			sprintf(buf, "Bad hour in time #%d", ip->data.time.tz_num);
			(*dl_error_proc)(buf);
			return(0);
		}

		if (ip->data.time.minutes > 59) {
			sprintf(buf, "Bad minute in time #%d", ip->data.time.tz_num);
			(*dl_error_proc)(buf);
			return(0);
		}

		if (ip->data.time.month < 1 || ip->data.time.month > 12) {
			sprintf(buf, "Bad month in time #%d", ip->data.time.tz_num);
			(*dl_error_proc)(buf);
			return(0);
		}

		if (ip->data.time.day < 1 || ip->data.time.day > 31) {
			sprintf(buf, "Bad day in time #%d", ip->data.time.tz_num);
			(*dl_error_proc)(buf);
			return(0);
		}
	...
	case DL_ALARM_TYPE:
		...
		if (ip->data.alarm.hours > 23) {
			sprintf(buf, "Bad hour in alarm #%d", ip->data.alarm.alarm_num);
			(*dl_error_proc)(buf);
			return(0);
		}

		if (ip->data.alarm.minutes > 59) {
			sprintf(buf, "Bad minute in alarm #%d", ip->data.alarm.alarm_num);
			(*dl_error_proc)(buf);
			return(0);
		}

		if (ip->data.alarm.month > 12) {
			sprintf(buf, "Bad month in alarm #%d", ip->data.alarm.alarm_num);
			(*dl_error_proc)(buf);
			return(0);
		}

		if (ip->data.alarm.day > 31) {
			sprintf(buf, "Bad day in alarm #%d", ip->data.alarm.alarm_num);
			(*dl_error_proc)(buf);
			return(0);
		}
	...
	case DL_APP_TYPE:
		...
		if (!dl_string_ok(ip->data.app.label, wi->max_str)) {
			sprintf(buf, "Bad characters in label in appointment.");
			(*dl_warn_proc)(buf);
		}
	...
	case DL_ANNIV_TYPE:
		...
		if (!dl_string_ok(ip->data.anniv.label, wi->max_str)) {
			sprintf(buf, "Bad characters in label in anniversary.");
			(*dl_warn_proc)(buf);
		}
